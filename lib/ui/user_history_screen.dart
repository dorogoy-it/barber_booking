                                        ...
                                        child: Column(children: [
                                          ...
                                          Column(
                                            children: [
                                              GestureDetector(
                                                onTap: (userBookings[index].done || isExpired)
                                                  ...
                                                  Alert(
                                                    ...
                                                    desc: 'Пожалуйста, удалите ещё и запись из календаря',
                                                    buttons: [
                                                      DialogButton(
                                              ...
                                              GestureDetector(
                                                onTap: !userBookings[index].done || DateTime.fromMillisecondsSinceEpoch(userBookings[index].timeStamp).add(const Duration(days: daysAfterBooking)).isBefore(syncTime)
                                                    ? null
                                                    : () {
                                                  if (userBookings[index].done) {
                                                    showReviewDialog(context, userBookings[index]);
                                                  }
                                                },
                                                child: Container(
                                                  decoration: const BoxDecoration(
                                                  ...
                                                  child: Center(
                                                    child: Text(
                                                      'Оставить отзыв',
                                                      style: GoogleFonts.robotoMono(
                                                        color: !userBookings[index].done || DateTime.fromMillisecondsSinceEpoch(userBookings[index].timeStamp).add(const Duration(days: daysAfterBooking)).isBefore(syncTime)
                                                            ? Colors.grey
                                                            : Colors.white,
                                                      ),
                                                    ),
...
void showReviewDialog(BuildContext context, BookingModel booking) async {
  ...
  final bookingTimeStamp = DateTime.fromMillisecondsSinceEpoch(booking.timeStamp);
  ...
  if (bookingTimeStamp.isAfter(currentTime)) {
    showDialog(
      ...
      builder: (context) {
        return AlertDialog(
          title: const Text('Ошибка'),
          content: Text('Вы можете оставить отзыв только после окончания записи, которая была запланирована на ${DateFormat('dd/MM/yyyy HH:mm').format(bookingTimeStamp)}'),
          actions: [
            TextButton(
              onPressed: () => Navigator.of(context).pop(),
              child: const Text('OK'),
