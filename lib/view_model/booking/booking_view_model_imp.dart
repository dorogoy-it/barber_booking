  ...
  void confirmBooking(WidgetRef ref, BuildContext context, GlobalKey<ScaffoldState> scaffoldKey) async {
    ...
    if (bookingExists) {
      ...
      batch.commit().then((value) {
        ...
        var notificationPayload = NotificationPayloadModel(topic: ref.read(selectedBarber.notifier).state.docId!,
            notification: NotificationContent(title: 'Новая запись ${ref.read(selectedTime.notifier).state} - ${DateFormat('dd/MM/yyyy').format(ref.read(selectedDate.notifier).state)}', body: 'К вам записался пользователь с номером ${FirebaseAuth.instance.currentUser!.phoneNumber!}'));
